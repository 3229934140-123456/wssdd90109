...
import type { Post, PostCategory, Sentiment } from '@/types'
import { CATEGORY_LABELS, SENTIMENT_LABELS } from '@/types'
import { MessageCircle, Eye, TrendingUp, TrendingDown, Minus, Clock } from 'lucide-react'
...
export default function PostCard({ post, onClick, disposalStatus }: PostCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="group relative flex cursor-pointer overflow-hidden rounded-xl border border-white/5 bg-[#16162e] transition-all duration-200 hover:border-white/10 hover:bg-[#1a1a36] hover:shadow-lg hover:shadow-black/20"
    >
          ...
          {disposalStatus && (
            <span
              className={cn(
                ...
                DISPOSAL_BADGES[disposalStatus]?.bg,
                DISPOSAL_BADGES[disposalStatus]?.text
              )}
            >
              {DISPOSAL_NAMES[disposalStatus]}
            </span>
          )}
        </div>
